...
const tokenWithCookies = (response, token) => {
  const cookieOpt = {
    ...
    expires: new Date(
      Date.now() + process.env.COOKIE_EXPIREING_TIME * 24 * 60 * 60 * 1000
    ),
...
exports.login = catchAsync(async (request, response, next) => {
  ...
  if (!email || !password)
    return next(
      new AppError(400, "Please provide your email and password to login !")
    );
...
exports.protect = catchAsync(async (request, response, next) => {
  ...
  if (
    ...
    token =
      request.headers.authorization.split(
        " "
      )[1]; // getting the token at second index of the array .
  // OR authenticate the user from the cookies
  else if (request.cookies.jwt) token = request.cookies.jwt;

  if (!token) {
    if (request.originalUrl.startsWith("/myProfile"))
      return response.redirect("/"); // this if() to prevent user go back to a protected route after logging out.
  ...
  if (!currentUser)
    return next(new AppError(401, "this user is no loger exist"));
  ...
  if (currentUser.changedPasswordAfter(payload.iat))
    ...
    return next(
      new AppError(
        401,
        "This User has recently changed the password, Please log in again :)"
      )
    );
  ...
  return (request, response, next) => {
    ...
    if (!roles.includes(request.user.role)) {
      return next(
        new AppError(
          403,
          "you don't have the premission to perform this action"
        )
      );
    }
    next();
...
exports.forgetPassword = catchAsync(async (request, response, next) => {
  ...
  const resetURL = `${request.protocol}://${request.get(
    "host"
  )}/api/vi/users/resetPassword/${randomToken}`;
  ...
  try {
    ...
    return next(
      new AppError(
        500,
        "An error has happend while sending the reset password email to the user ... Please try again"
      )
    );
  }
});
exports.resetPassword = catchAsync(async (request, response, next) => {
  ...
  }); // this line will find the specific user and check the token availability at once, if the time expired then will return a false to the next if() .

  // 2- Setting the new password ONLY IF THE TOKEN TIME HASN'T EXPIRED AND THE USER IS EXIST:
  if (!user)
    return next(new AppError(400, "Reset passowrd's link has expired"));
...
exports.updateMyPassword = catchAsync(async (request, response, next) => {
  ...
  if (!(await user.comparePassword(providedPassword, user.password))) {
    ...
    return next(
      new AppError(
        401,
        "The provided password does not match the current one. Please make sure to write it correctly"
      )
    );
  }
